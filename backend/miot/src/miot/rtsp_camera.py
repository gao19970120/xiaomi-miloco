...
class RtspCameraInfo(BaseModel):
    ...
    home_id: Optional[str] = Field(default=None, description="Bound home id")
...
class RTSPCameraInstance:
    ...
    def __init__(
        ...
        self._audio_codec_by_channel: Dict[int, MIoTCameraCodec] = {}
    ...
    def get_audio_codec(self, channel: int = 0) -> Optional[MIoTCameraCodec]:
        """Return the last audio codec observed on a channel."""
        return self._audio_codec_by_channel.get(channel)
    ...
    async def start_async(
        ...
        for _ in range(channel_count):
            decoder = MIoTMediaDecoder(
                ...
                # The camera runs at ~15fps, while Miloco's overview and
                # perception pipeline only need a smooth, bounded 10fps feed.
                # MSE/JMuxer needs enough early samples to leave HAVE_NOTHING
                # quickly; 5fps can still spend several seconds buffering.
                # Throttle before ndarray conversion to avoid wasting CPU.
                video_frame_interval=100,
    ...
    async def register_decode_video_frame_async(
        self,
        callback: Callable[[str, Any, int, int, int, int], Coroutine],
        channel: int = 0,
        multi_reg: bool = False,
    ) -> int:
        """Register a decoded BGR ndarray callback."""
        await self.__update_raw_data_register_status_async(channel=channel)
        reg_key = f"decode_video_frame.{channel}"
        self._callbacks.setdefault(reg_key, {})
        reg_id = len(self._callbacks[reg_key]) if multi_reg else 0
        self._callbacks[reg_key][str(reg_id)] = callback
        return reg_id
    ...
    async def unregister_decode_video_frame_async(
        ...
        """Unregister a decoded BGR ndarray callback."""
        reg_key = f"decode_video_frame.{channel}"
        if reg_key in self._callbacks:
            self._callbacks[reg_key].pop(str(reg_id), None)
    ...
    async def __update_raw_data_register_status_async(self, channel: int, is_register: bool = True) -> None:
        ...
        if is_register and reg_key not in self._callback_refs:
            ...
            if len(self._callbacks.get(f"decode_jpg.{channel}", {})) > 0:
                need_unreg = False
            if len(self._callbacks.get(f"decode_video_frame.{channel}", {})) > 0:
                need_unreg = False
    ...
    def __on_raw_data(self, frame_header_ptr: Any, data: bytes) -> None:
        ...
        # Keep the configured/runtime *video* codec stable. Audio packets share
        # this callback and must not replace camera_info.codec with G711/Opus.
        if codec_id in [MIoTCameraCodec.VIDEO_H264, MIoTCameraCodec.VIDEO_H265] and self._camera_info.codec != codec_id:
        ...
        if codec_id in [MIoTCameraCodec.VIDEO_H264, MIoTCameraCodec.VIDEO_H265]:
            if (
                self._callbacks.get(f"decode_jpg.{channel}")
                or self._callbacks.get(f"decode_video_frame.{channel}")
            ):
            ...
            self._audio_codec_by_channel[channel] = codec_id

            # The shared MIoT decoder currently only initializes an Opus
            # decoder. Sending G.711 packets into it leaves _audio_decoder=None
            # and produces one exception per packet, so G.711 is decoded by the
            # RTSP bridge below. Opus continues through the PyAV decoder.
            if codec_id == MIoTCameraCodec.AUDIO_OPUS:
                try:
                    self._decoders[channel].push_audio_frame(frame_data)
                except Exception as e:
                    _LOGGER.warning(
                        "RTSP audio push_audio_frame failed (ch=%d): %s", channel, e
                    )

            # G.711 is lightweight and deterministic to decode directly. Opus
            # callbacks are emitted later by __on_audio_decode_callback.
            d_callbacks = self._callbacks.get(f"decode_pcm.{channel}", {})
            if codec_id in [MIoTCameraCodec.AUDIO_G711A, MIoTCameraCodec.AUDIO_G711U]:
                for d_callback in list(d_callbacks.values()):
                    asyncio.run_coroutine_threadsafe(
                        d_callback(
                            self._camera_info.did,
                            frame_data.data,
    ...
    async def __on_video_frame_decode_callback(
        ...
        """Fan out decoded BGR frames without the JPEG preview throttle."""
        callbacks = self._callbacks.get(f"decode_video_frame.{channel}", {})
