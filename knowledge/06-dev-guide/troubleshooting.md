...
```bash
miloco-cli doctor              # 自动从后端拉取摄像头清单，逐台主动探测网络可达性
miloco-cli doctor --device-ip <IP>   # 只探测指定 IP
```

`doctor` 分三段输出（主机环境 / Miloco 运行状态含 backend 版本 / 检测项）：除防火墙、容器、WSL 网络判定外，会对每台有 LAN IP 的摄像头做分层主动探测（网段匹配 → 路由出接口 → L3 ping/ARP → UDP 出站），把结论定位到具体失败环节。`--json` 输出结构化结果供脚本消费（有 FAIL 项时退出码为 1），`--lang en` 切英文（默认 zh）。命令实现在 `cli/src/miloco_cli/commands/doctor.py`，文案表在 `doctor_i18n.py`。
